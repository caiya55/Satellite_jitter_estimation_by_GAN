# -*- coding: utf-8 -*-
"""
Created on Fri Aug 10 15:47:55 2018
...
parser.add_argument("--alpha", type=float, help="choose the value of alpha", default = 0.25)
parser.add_argument("--max_pooling", type=bool, help="choose whether max_pooling is used", default = False)
...
#  data = load__class_images_with_C('..\\..\\dataset\\image_deform\\solar_panel\\',n_images,istrain = True) # load the images from different classes
  data = load__class_images_with_C('..\\..\\dataset\\image_deform\\farms\\',n_images,istrain = True) # load the images from different classes
...
#  z_train = z_train[:,:,:,np.newaxis]
  ...
  data = load__class_images_with_C('..\\..\\dataset\\image_deform\\farms\\',n_images,istrain = False)# load the images from different classes
...
##      reg.deploy(config.tmp_dir, batch_x, batch_y)
    ...
    if (i+1) % 20 == 0:
        ...
        plt.show()
#        
#        yaogan_x = preprocess('..//..//dataset//yaogan26//for_classification.png', bo, bo)
...
#        cv2.imwrite('results//farm_result.png', uint_img(output[:,:,0]))
#
#        plt.show()            
...
#        plt.ylabel('Pixel')
#        plt.axis('off')
#        plt.savefig('results//cross-track-curve.png')
#        plt.show()
#        np.save('farm_jitter.npy', wrap_yaogan)
#        plt.plot(wrap_yaogan[:,1])
