# -*- coding: utf-8 -*-
"""
Created on Tue Oct 23 20:01:15 2018
...
def psnr(img1, img2):
    ...
    for k in range(img1.shape[0]):
        mse = np.mean( (img1[k] - img2[k]) ** 2 )
        if mse == 0:
            return 100
        PIXEL_MAX = 255.0
        psnr_k = 20 * math.log10(PIXEL_MAX / math.sqrt(mse))
        psnr_out.append(psnr_k)
    return np.mean(psnr_out)
parser = argparse.ArgumentParser()
parser.add_argument("--final_layer", type=int, help="choose the number of final layers", default = 128)
parser.add_argument("--alpha", type=float, help="choose the value of alpha", default = 0.5)
...
#  dh = MNISTDataHandler("MNIST_data", is_train=True)
  data = load__class_images_with_C('..\\..\\dataset\\image_deform\\freeway\\',300,istrain = True) # load the images from different classes
...
#  z_train = z_train[:,:,:,np.newaxis]
  ...
  data = load__class_images_with_C('..\\..\\dataset\\image_deform\\freeway\\',400,istrain = False) # load the images from different classes
...
#    reg.evaluate(batch_x, batch_y, i)    
    if (i+1) % 3 == 0:
        for index in range(int(x_test.shape[0] / batch_size)):
            ...
            loss, y_result, _ = restore.predict(batch_x, batch_y, batch_z)  
            psnr_i = psnr(y_result[:,5:-5,5:-5,0], batch_y[:,5:-5,5:-5,0])
        ...
        print("test iter {:>6d} : {} : {}".format(i+1, value, psnr_mean))
