...
#        img = img / 255
    img = (img - 127.5) / 127.5
    num = 5
    out = np.zeros([5,296,296,1])
    for i in range(num):
        out[i,:,:,0] = img
    return out

def preprocess2(path, x_b, y_b):
    img = load_image(path)
#    cv2.imres
    img = np.array(img) 
#    if randint(0,1)==0:
...
parser.add_argument("--alpha", type=float, help="choose the value of alpha", default = 0.75)
...
#  data = load__class_images_with_C('..\\..\\dataset\\patternNet images\\image_deform\\solar_panel\\',600,istrain = True) # load the images from different classes
  data = load_images('..\\..\\dataset\\patternNet images\\image_deform\\',400,istrain = True) # load the images from different classes

  y_train, x_train, z_train = data['B'], data['A'], data['C']
  x_train = x_train[:,:,:,np.newaxis]
  y_train = y_train[:,:,:,np.newaxis] 
  # load the test dataset
#  data = load__class_images_with_C('..\\..\\dataset\\patternNet images\\image_deform\\solar_panel\\',100,istrain = False) # load the images from different classes
  data = load_images('..\\..\\dataset\\patternNet images\\image_deform\\',100,istrain = False)# load the images from different classes
...
#    reg.evaluate(batch_x, batch_y, i)    
    if (i+1) % 1 == 0:
        for index in range(int(x_test.shape[0] / batch_size)):
            ...
            batch_z = z_test[index*batch_size:(index+1)*batch_size]
...
##      reg.deploy(config.tmp_dir, batch_x, batch_y)
    ...
    if (i+1) % 1 == 0:
        img_i = 4
#        test_img = preprocess('results//pale3.jpg', 0, 0)
#        batch_x, batch_z = jitter2D(test_img)  
#        batch_x = batch_x[:,bo:bo+256,bo:bo+256,:] 
#        batch_z = batch_z[:,bo:bo+256,:]
        _,_, y_result, wrap_test = restore.predict_one(batch_x[img_i], config)
        plt.plot(batch_z[img_i,:,:])
        plt.plot(wrap_test[:,:])
...
#        plt.imshow(batch_y[img_i,bo:bo+256,bo:bo+256,0],cmap='gray')
#        plt.grid(False)
#        plt.axis('off')
#        plt.title('Raw image')
#        plt.show()   
#        
#        plt.imshow(x_test[img_i,bo:bo+256,bo:bo+256,0],cmap='gray')
#        plt.grid(False)
#        plt.axis('off')
